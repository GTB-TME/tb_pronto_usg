...
output$prov_usaid_plot <- renderEcharts4r({
  ...
  pretrump_year_avge <- ifelse(frequency == 71, pretrump/4, pretrump/12) |> as.numeric()
  ...
  data_to_plot <- data_to_plot |>
    filter(year >= ifelse(pretrump_year<2024,2024,pretrump_year))
  ...
  prov_chart <-  data_to_plot |>
    ...
    e_mark_line(data = list(yAxis = pretrump_year_avge),
  ...
  max_value <- data_to_plot |>
    ...
    max(na.rm = TRUE) |>
    max(c(pretrump_year_avge), na.rm = TRUE)
  # 
  if (max_value == pretrump_year_avge) {
